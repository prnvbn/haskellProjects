...
move 'F' _ ((x,y),orientation)
  = ((x',y'),orientation)
  where
      x' = x+cos (degToRad orientation)
      y' = y+sin (degToRad orientation)
move 'R' angle (vertex,orientation)
  = (vertex,newOrientation)
  where
    newOrientation = mod' (orientation - angle) 360.0
...
-- Uses the implicit savedState parameter to deal with bracketed commands
trace1 :: Commands -> Angle -> Colour -> [ColouredLine]
trace1 cs angle colour
  = trace1' cs origin origin
  where
    origin = ((0.0,0.0),90.0)
    trace1' :: Commands -> TurtleState -> TurtleState -> [ColouredLine]
    trace1' [] _ _
      = []
    trace1' (']':cs) tState savedState
      = trace1' cs savedState savedState
    trace1' ('[':cs) tState savedState
      = trace1' cs tState tState
    trace1' (c:cs) tState@(oldVertex,_) savedState
      | c == 'F'  = (oldVertex,newVertex,colour):trace1' cs newTState savedState
      | otherwise = trace1' cs newTState savedState
      where
...
-- inverts the colour at every turn in the turtle state
trace2VarColour :: Commands -> Angle -> Colour -> [ColouredLine]
trace2VarColour commands angle colour
  = trace2' commands [] origin colour
  where
    origin = ((0.0,0.0),90.0)
    trace2' :: Commands -> Stack -> TurtleState -> Colour -> [ColouredLine]
    trace2' "" _ _ _
      = []
    trace2' commands@(c:cs) stack tState@(oldVertex,_) colour
      |c == '['  = trace2' cs (tState:stack) tState colour
      |c == ']'  = trace2' cs rest top colour
      |c == 'F'  = (oldVertex,newVertex,colour):trace2' cs stack newTState invColour
      |otherwise = trace2' cs stack newTState colour
      where
        invColour  = (1-r,1-g,1-b)
        (r,g,b)    = colour
        (top:rest) = stack
        newTState@(newVertex,_) = move c angle tState
...
drawLSystem2' :: LSystem -> Int -> Colour -> IO ()
drawLSystem2' system n colour
  = drawLines (trace2VarColour (expandLSystem system n) (angle system) colour)
